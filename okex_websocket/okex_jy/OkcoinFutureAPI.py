...
from okex_jy.HttpMD5Util  import buildMySign,httpGet,httpPost
...
class OKCoinFuture:
    ...
    # OKex kline信息
    def future_kline(self, symbol, contractType):
        FUTURE_TICKER_RESOURCE = "/api/v1/future_kline.do"
        params = ''
        if symbol:
            params += '&symbol=' + symbol + '&type=1min' if params else 'symbol=' + symbol + '&type=1min'
        if contractType:
            params += '&contract_type=' + contractType if params else 'contract_type=' + symbol
        return httpGet(self.__url, FUTURE_TICKER_RESOURCE, params)
    ...
    # 期货获取详细订单信息
    def future_ordersinfo(self, symbol, contractType, orderId):
        FUTURE_ORDERINFO = "/api/v1/future_orders_info.do?"
        params = {
            'api_key': self.__apikey,
            'symbol': symbol,
            'contract_type': contractType,
