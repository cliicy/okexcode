...
def download(self,evt,sym):
    ...
    if not os.path.exists(path_txt):
        ...
        if evt['channel'] == 'addChannel':
            ...
            # 将原始文件写入txt
            ok_path = path_txt + '/ticker_' + today_date + '.txt'
            ...
            if evt['channel'] == 'addChannel':
                ...
                # 解析获得文件的格式
                result = {}
                result['symbol'] = re.match('.*_future(.*)_ticker.*', evt['channel']).group(1)
                ...
                # 设置存储路径
                file_name = path_txt + '/ticker_' + today_date + '.csv'
...
def on_message(self,evt):
    ...
    if rdata['channel'] == 'addChannel':
        ...
        # 调用小兵mq
        sender = MqSender('okcoin_future', 'ticker')
        sender.send(evt)
