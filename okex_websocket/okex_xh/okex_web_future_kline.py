    ...
    if not os.path.exists(path_txt):
        ...
        if evt['channel'] == 'addChannel':
            ...
            if evt['channel'] == 'addChannel':
                ...
                # 获得币种 合约类型
                symbol = re.match('.*_future(.*)_kline.*', evt['channel']).group(1) + re.match('.*_kline_(.*)_1min', evt['channel']).group(1)
                # print(symbol)
                #获得合约类型
                ...
                a = evt['data']
                i = a[0]
                line = [re.match('.*_future(.*)_kline.*', evt['channel']).group(1),re.match('.*_kline_(.*)_1min', evt['channel']).group(1),
                ...
                if symbol not in result:
                    ...
                    if ''.join(line[0:2]) == ''.join(result[symbol][0:2]) and line[2] == result[symbol][2]:
                        ...
                        if os.path.exists(file_name):
                            ...
                            with open(file_name, 'a+', encoding='utf-8', newline='') as f:
                                writer = csv.writer(f)
                                header = ['symbol','contractType', 'ts', 'tm_intv', 'id', 'open', 'close', 'low', 'hight', 'amount',
                                          'vol',
                                          'count']
