...
from python.HttpMD5Util import buildMySign,httpGet,httpPost

class OKCoinSpot:

    def __init__(self,url,apikey,secretkey):
        self.__url = url#取值地址
        self.__apikey = apikey#api在网址获取
        self.__secretkey = secretkey#secret
    ...
    # 获取OKCOIN kline信息
    def kline(self, symbol='',since=''):
        TICKER_RESOURCE = "/api/v1/kline.do"
        params = ''
        if symbol:
            params = 'symbol=%(symbol)s&since=%(since)s&type=1min&size=1440' % {'symbol': symbol,'since':since}
        return httpGet(self.__url, TICKER_RESOURCE, params)

    #获取OKCOIN现货市场深度信息
    def depth(self,symbol = ''):
        DEPTH_RESOURCE = "/api/v1/depth.do"#https://www.okcoin.com/api/v1/depth.do?symbol=btc_usd
        params=''
        if symbol:
            params = 'symbol=%(symbol)s' %{'symbol':symbol}
