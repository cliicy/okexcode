...
class OK_download(object):
    ...
    def download(self,sym):
        ...
        rs_eos_usdt = okcoinSpot.depth(sym)
        #print('eos_usdt')
        # print(rs_eos_usdt)
        # 原始数据写入txt depth_20180724.txt
        # 判断文件夹是否存在
        self.create_file()
        path = self.path_txt + '/depth_'+ sym +'_' + self.today_date + '.txt'
    ...
    def create_exccl(self):
        ...
        #获取数据
        #卖出价格数据,买入价格数据,
        rs_eos_usdt_asks,rs_eos_usdt_bids = self.download(OK.sym)
        #获取爬取时间
        t = [str(self.ts)]
        #现货深度卖出开始-------
        # 设置存储路径
        #file_name = self.path + u'depth_eos_usdt_' + self.today_date + '.csv'
        file_name = self.path_txt + '/depth_'+ OK.sym +'_' + self.today_date + '.csv'
        #设置一个空列表接收整合好的数据
        print(file_name)
        result = []
        if os.path.exists(file_name):
            with open(file_name, 'a+', encoding='utf-8', newline='') as f:
                rs_eos_usdt_bids = [[OK.sym] + t + [i] + value for i, value in enumerate(rs_eos_usdt_bids)]
                # 卖出
            ...
            # 设置现货深度卖出表头
            rs_eos_usdt_asks_name = ['symbol','ts','depth', 'sell_price','sell_amt', 'buy_price', 'buy_amt']
