...
class OK_download(object):
    def __init__(self):
        ...
        self.path_txt = self.path_file + '/okex_rest' + '/trades'
    # 创建文件夹
    ...
    def download(self,sym,contractType):
        ...
        rs_bch_usdt = okcoinFuture.future_trades(sym,contractType)
        # 原始数据写入txt depth_20180724.txt
        # 判断文件夹是否存在
        self.create_file()
        path = self.path_txt + '/trades_'+ sym +'_'+ contractType +'_' + self.today_date + '.txt'
    ...
    def create_exccl(self):
        ...
        # 设置存储路径
        file_name = self.path_txt + '/trades_'+ OK.sym +'_'+ OK.contractType +'_' + self.today_date + '.csv'
        #设置存放格式
        result = {}
